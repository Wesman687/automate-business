import     @staticmethod
    def get_customer_base_dir(customer: User) -> str:
        """Generate base directory path for customer files"""
        safe_email = customer.email.replace('@', '_').replace('.', '_')
        return os.path.join("uploads", "customers", f"customer_{customer.id}_{safe_email}")
    
    @staticmethod
    def create_customer_directories(customer: User) -> dict: typing import Optional
