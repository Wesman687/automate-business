...
export default function FinancialPage() {
  ...
  const fetchFinancialData = async () => {
    try {
      ...
      const [invoicesRes, recurringRes, summaryRes] = await Promise.all([
        ...
        api.get<RecurringPayment[]>('/recurring-payments'),
        api.get<FinancialSummary>('/financial-summary')
      ]);

      // Handle API responses - they should be arrays/objects directly
      const invoicesData = Array.isArray(invoicesRes) ? invoicesRes : [];
      const recurringData = Array.isArray(recurringRes) ? recurringRes : [];
  ...
  const createInvoice = async (invoiceData: InvoiceCreateRequest) => {
    try {
      const token = localStorage.getItem('admin_token');
  ...
  const createRecurringPayment = async (paymentData: RecurringPaymentCreateRequest) => {
    try {
      const token = localStorage.getItem('admin_token');
      await api.post('/recurring-payments', paymentData);
      await fetchFinancialData(); // Refresh the data
    } catch (error) {
          ...
          <div className="bg-white p-6 rounded-lg shadow border">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{formatCurrency(summary.total_invoiced)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow border">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{formatCurrency(summary.monthly_recurring_revenue)}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg shadow border">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {recurringPayments.filter(p => p.status === 'active').slice(0, 5).map((payment) => (
