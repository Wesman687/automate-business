...
export default function Dashboard() {
  ...
  const adaptChangeRequestForModal = (
    request: CustomerChangeRequest
  ): ChangeRequestModalAdapter => ({
    id: request.id,
    customer_id: request.customer_id,
    title: request.title,
    description: request.description,
  ...
  const adaptAppointmentForModal = (
    appointment: Appointment
  ): AppointmentModalAdapter => ({
    id: appointment.id,
    ...
    appointment_time:
      appointment.scheduled_date.split("T")[1]?.split(".")[0] || "00:00:00", // Extract time part
    ...
    try {
      const [overviewR, requestsR, appointmentsR, sessionsR, emailsR] =
        await Promise.allSettled([
          ...
          api.get<{ change_requests: CustomerChangeRequest[] }>(
            "/change-requests"
          ),
      ...
      if (overviewR.status === "fulfilled" && overviewR.value) {
        ...
        if (
          ...
          Array.isArray(overview.upcoming_appointments)
        ) {
          // Transform the overview appointments to match our Appointment type
          const transformedAppointments = overview.upcoming_appointments.map(
            (apt: any) => ({
              id: apt.id,
      ...
      if (
        appointmentsR.status === "fulfilled" &&
        (overviewR.status !== "fulfilled" ||
          !overviewR.value?.upcoming_appointments)
      ) {
        const list = Array.isArray(appointmentsR.value)
          ? appointmentsR.value
          : [];
      ...
      if (emailsR.status === "fulfilled") {
        ...
        setStats((prev) => ({
          ...prev,
          unread_emails:
            emailsR.value.count ?? emailsR.value.emails?.length ?? 0,
        }));
      }
    } catch (e) {
  ...
  const handleSaveRequest = async (
    updated: Partial<ChangeRequestModalAdapter>
  ) => {
    try {
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-lg border border-white/10 p-6">
          ...
          <div className="space-y-3">
            {pendingRequests.length === 0 ? (
              <p className="text-gray-400">No pending change requests</p>
            ) : (
              pendingRequests
                .slice(0, 3)
                .map((request) => (
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-lg border border-white/10 p-6">
          ...
          <div className="space-y-3">
            {newChatLogs.length === 0 ? (
              <p className="text-gray-400">No unseen chat sessions</p>
            ) : (
                ...
                >
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                      Hello, I need help automating my business
                    </span>
                    <button className="text-xs text-green-400 bg-green-400/20 px-2 py-1 rounded">
                      Mark as Seen
                    </button>
                  </div>
                </div>
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-lg border border-white/10 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                onClick={() => setShowEmailManager(true)}
                className="text-xs text-purple-400 bg-purple-400/20 px-2 py-1 rounded hover:bg-purple-400/30 transition-colors"
              >
                Email Manager
              </button>
            </div>
          ...
          <div className="space-y-3">
            {stats.unread_emails === 0 ? (
              ...
              <p className="text-gray-400">
                Click &quot;Email Manager&quot; to view unread emails
              </p>
            )}
          </div>
        </div>
      ...
      {showAppointmentModal && (
        <SmartAppointmentModal
          ...
          appointment={
            editingAppointment
              ? adaptAppointmentForModal(editingAppointment)
              : null
          }
        />
      )}
