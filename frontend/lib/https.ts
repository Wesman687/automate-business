...
const PRODUCTION_EMAIL_SERVER = "https://server.stream-lineai.com";
...
export function buildBackendUrl(path: string) {
  ...
  // Email endpoints ALWAYS go to production server - never local/dev
  // Check both with and without leading slash
  if (path.startsWith("email/") || path.startsWith("/email/")) {
    return `${PRODUCTION_EMAIL_SERVER}/${path.replace(/^\/+/, "")}`;
  }
...
export function buildProxyUrl(path: string) {
  ...
  // Email endpoints ALWAYS go to production server - bypass proxy entirely
  // Check both with and without leading slash
  if (path.startsWith("email/") || path.startsWith("/email/")) {
    return `${PRODUCTION_EMAIL_SERVER}/${clean}`;
  }
