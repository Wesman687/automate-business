...
class CreditSDK {
     ...
     */
    async getCreditBalance(requiredCredits) {
     ...
     */
    async hasSufficientCredits(requiredCredits) {
        try {
     ...
     */
    async consumeCredits(credits, service, description, metadata) {
        if (!this.authSDK.isAuthenticated()) {
    ...
    async getCreditPackages() {
        ...
        const response = await fetch(`${this.apiBase}/api/cross-app/credits/packages?session_token=${sessionToken}&app_id=${this.authSDK['config'].appId}`);
        if (!response.ok) {
            const errorData = await response.json();
    ...
    async getUserSubscriptions() {
        ...
        const response = await fetch(`${this.apiBase}/api/cross-app/credits/subscriptions?session_token=${sessionToken}&app_id=${this.authSDK['config'].appId}`);
        if (!response.ok) {
            const errorData = await response.json();
     ...
     */
    async validateCreditRequirements(requiredCredits, service) {
        try {
            const balance = await this.getCreditBalance(requiredCredits);
            return {
     ...
     */
    async getRecommendedPackage(targetCredits) {
        try {
     ...
     */
    calculateCostPerCredit(creditPackage) {
     ...
     */
    async quickCreditCheck(requiredCredits) {
        try {
            const balance = await this.getCreditBalance(requiredCredits);
...
exports.CreditSDK = CreditSDK;
//# sourceMappingURL=CreditSDK.js.map